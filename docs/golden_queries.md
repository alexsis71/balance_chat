...
2. Полный баланс на дату, только план.
3. Полный баланс на дату, только факт.
4. Раздел «Ресурсы» на дату.
5. Раздел «Поступление» на дату.
6. Раздел «Распределение» на дату.
7. Одна canonical статья на дату.
8. Сумма flow-статьи за календарный месяц.
9. Среднее flow-статьи за период с правильной зернистостью.
10. Собственные нужды на дату.
11. Производство за месяц.
12. Запас как point-in-time, без суммирования дней.
13. Закачка ПХГ за месяц.
14. Отбор ПХГ за месяц.
15. Поступление от указанного source business object.
16. Распределение в указанный destination business object.
17. Распределение в указанный GEO object.
18. Продолжение «а на следующий день?» с наследованием объекта.
19. Продолжение «а план?» с наследованием даты и статьи.
20. Продолжение «а факт?» с наследованием даты и статьи.
21. Смена статьи с сохранением явно установленного периода.
22. Смена баланса с сохранением явно установленной даты.
23. Strict `no_data`, не разрушающий последний успешный scope.
24. Действительно неоднозначная статья с typed clarification и без DB-запроса.
...
- `result_shape`, `operation`, `metric`, `value_mode = plan|fact|both`;
...
иерархических строк, plan/fact/unit и контрольные значения трёх canonical
статей.
...
- возвращает только `fact_value`, без `plan_value` и публичной единицы в строке.
