...
`GQ-001…GQ-008`; новые записи добавляются только отдельным решением.
...
| BC-02 | Направленный транспорт и reverse | Декомпозировать; атомарное сравнение двух направлений утверждено как GQ-008 | P0 Core для утверждённой пары, остальное P1 |
...
1. Полный баланс на одну дату (`GQ-001`, утверждён).
2. Раздел «Ресурсы» на дату (`GQ-002`, утверждён).
3. Раздел «Поступление» на дату (`GQ-003`, утверждён).
4. Раздел «Распределение» на дату (`GQ-004`, утверждён).
...
16. Сравнение двух противоположных business-направлений за месяц (`GQ-008`, утверждён).
17. Продолжение «а на следующий день?» с наследованием объекта.
18. Смена статьи с сохранением явно установленного периода.
19. Смена баланса с сохранением явно установленной даты.
20. Strict `no_data`, не разрушающий последний успешный scope.
21. Действительно неоднозначная статья с typed clarification и без DB-запроса.
...
## GQ-002…GQ-004: иерархические разделы

Следующий P0-блок утверждает запросы «Покажи раздел Ресурсы/Поступление/
Распределение баланса ГП ТГ Москва за 25.06.2025». Все три контракта требуют:

- `result_shape=section_snapshot`, `metric=balance_section`;
- canonical balance `BAL:2010000039953` и отдельную root article раздела;
- период `[2025-06-25, 2025-06-26)`;
- один `api.show_balance_day`, без article parameters и повторного DB-запроса;
- metadata-bound фильтрацию по `ArticleRecord.path`, исходный порядок строк и
  rebased hierarchy, где корень выбранного раздела имеет indent 0;
- только `fact_value` в `тыс. м3`.

На фиксированной БД ожидаются 16 строк для «Ресурсы», 15 для «Поступление» и
71 для «Распределение». Контрольные значения и позиции строк закреплены в TOML.
Полный balance API выбран намеренно: плоский `show_balance_articles_day` не
возвращает имена и иерархический порядок. Фильтрация выполняется после одной DB
операции, но до удаления внутренних canonical IDs из публичного результата.

При выборе controls выявлено существующее presentation-расхождение для
`ART:2010000039727`: canonical metadata содержит незакрытую кавычку, а DB display
name — закрытую. Metadata не исправлялась; control выбран на другой однозначной
статье того же поддерева (`ART:2010000039721`).

Запущенный до обновления процесс воспроизводит исходный дефект: «Ресурсы» и
«Поступление» деградируют до ошибки/одной строки, «Распределение» — до scalar
...
## GQ-008: сравнение противоположных business-направлений

`GQ-008` утверждает один общий период `[2025-06-01, 2025-07-01)` и два
самостоятельных directional operand:

- `BAL:2010000040579` → `BAL:2010000040765` через
  `ART:2010000040514` (`ТГ Томск`);
- `BAL:2010000040765` → `BAL:2010000040579` через
  `ART:2010000040604` (`ТГ Сургут`).

Повторное использование двух business objects в противоположных ролях
разрешается на уровне operands, а не схлопывается в глобальный набор entities.
Native planner создаёт ровно две scalar-задачи и выполняет два
`api.show_balance_articles_period`; скрытого flow-balance fallback и повторного
interpretation-вызова нет. На фиксированной БД июньские суммы равны
`2512.377` и `9136.25 тыс. м3`. Golden проверяет canonical роли, статьи,
период, bounded SQL evidence, оба факта и `source_execution_count=2`.
