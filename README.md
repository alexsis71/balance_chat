...
Доменный контракт V2 является fact-only: плановых данных и режимов
`plan`/`both` не существует. Все исходные объёмные значения на входе, в
контексте, execution evidence и публичном результате имеют единственную
единицу `тыс. м3`; `%` допустим только для производного процентного расчёта.
